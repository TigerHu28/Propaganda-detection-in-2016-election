...
df2<-df2[,c(1:8,91:95,9:90)] #sorts the columns by interest
...
### Exploratory Data Analysis (EDA) ###
    ...
    ggplot(data = df2, mapping = aes(x = AdWordCount, colour = state_po)) +
    ...
    ggplot(data = df2) +
      geom_histogram(mapping = aes(x = (connectivity)^-1, binwidth = .01))
    
    ggplot(data = df2) +
      geom_point(mapping = aes(x = (connectivity)^-1, y = CTR*100))
    
    ggplot(data = df2, mapping = aes(x = (connectivity)^-1, y = CTR*100)) + 
      geom_boxplot(mapping = aes(group = cut_width((connectivity)^-1, .01)))
...
### Box-Cox: Transformation for non-linearity ###
    ...
    ClickEfficacy1_tr <- lm((CTR*100)^.38 ~ AdWordCount + AdDuration + Ad_Freq_state + ElectYear + `Alabama` + 
    ...
    summary(ClickEfficacy1_tr)
    plot(ClickEfficacy1_tr)
    
    library(olsrr)
    ols_plot_cooksd_bar(ClickEfficacy1_tr)
    ols_plot_dfbetas(ClickEfficacy1_tr)
    ols_plot_dffits(ClickEfficacy1_tr)
    ols_plot_resid_lev(ClickEfficacy1_tr)  

    # Jacknife proceedure #
    library(lmboot)
    formula
    JackObj <- jackknife(((CTR*100)^.38) ~ AdWordCount + AdDuration + Ad_Freq_state + ElectYear, data = df2) #perform the jackknife
    ...
    step1 <- stepAIC(ClickEfficacy1_jk, direction="both")
...
### Extract Coefficients for Secondary model with demographics
    ...
    model <- ddply(df2_1, "state", function(x) {
      model <- lmrob((CTR*)^.38 ~ AdWordCount + AdDuration + Ad_Freq_state + ElectYear + `Alabama` + `Arkansas` + `Arizona` + 
