...
### Extract Coefficients for Secondary model with demographics
    ...
    statedf = statedf[,c(2,5,8,11,41,44,50,56,59,62,77,82)]
    statedf$ctr = statedf$`ClickEfficacy1_jk[[1]]`
    for ( col in 1:ncol(statedf)){
      colnames(statedf)[col] <-  sub("X", "", colnames(statedf)[col]) #Remove unnecessary X from columns
    }
