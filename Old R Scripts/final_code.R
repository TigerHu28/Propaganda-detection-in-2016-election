...
##### Jan's Map #####
library(tidyverse) #loading in the libraries necessary
library(plotly)
library(sf)
library(maps)
library(ggplot2)
...
library(tm)
library(scales)
aa<-df2[c('state','AdText')] #subsetting wanted columns from df2
aa<-arrange(aa,state) #organizing aa by state
aa$AdText<-tolower(aa$AdText) #making the entire state column lowercase
aa$AdText<-gsub('[[:punct:]]','',aa$AdText) #getting rid of all punctuation

stopwords_regex<-paste(stopwords('en'), collapse = '\\b|\\b') #making a code to get rid of all english stopwords
stopwords_regex<-paste0('\\b', stopwords_regex, '\\b') #making a code to get rid of all english stopwords

fun3<-function(x){
  aa$AdText<-stringr::str_replace_all(aa$AdText, stopwords_regex, '')
  var1<-gsub('[[:punct:]]','',toString(aa$AdText[which(aa$state==x)]))
  var2<-sort(rowSums(as.matrix(TermDocumentMatrix(Corpus(VectorSource(var1))))),decreasing=TRUE)
  data.frame(word=names(var2),freq=var2)
} #making a function to run stopwords_regex 

statenames<-unique(aa$state) #getting the unique state names
wordlist3<-lapply(statenames,fun3) #running the function to get rid of stopwords and making a list by states, takes about two to three minutes to run
names(wordlist3)<-unique(aa$state) #labels the list elements by states

states<-map_data('state') #subsets a dataframe with all us states
states$five_words<-NA #creates a new column with NA

states[c(states$region=="alabama"),]$five_words<-c(paste(as.character(wordlist3[['Alabama']][c(1:5),c(1)]),collapse=', ')) #inserts the top five words for each state from the list
states[c(states$region=="arizona"),]$five_words<-c(paste(as.character(wordlist3[['Arizona']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="arkansas"),]$five_words<-c(paste(as.character(wordlist3[['Arkansas']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="california"),]$five_words<-c(paste(as.character(wordlist3[['California']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="connecticut"),]$five_words<-c(paste(as.character(wordlist3[['Connecticut']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="delaware"),]$five_words<-c(paste(as.character(wordlist3[['Delaware']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="district of columbia"),]$five_words<-c(paste(as.character(wordlist3[['District of Columbia']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="florida"),]$five_words<-c(paste(as.character(wordlist3[['Florida']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="georgia"),]$five_words<-c(paste(as.character(wordlist3[['Georgia']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="idaho"),]$five_words<-c(paste(as.character(wordlist3[['Idaho']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="illinois"),]$five_words<-c(paste(as.character(wordlist3[['Illinois']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="iowa"),]$five_words<-c(paste(as.character(wordlist3[['Iowa']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="kansas"),]$five_words<-c(paste(as.character(wordlist3[['Kansas']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="louisiana"),]$five_words<-c(paste(as.character(wordlist3[['Louisiana']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="maryland"),]$five_words<-c(paste(as.character(wordlist3[['Maryland']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="massachusetts"),]$five_words<-c(paste(as.character(wordlist3[['Massachusetts']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="michigan"),]$five_words<-c(paste(as.character(wordlist3[['Michigan']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="minnesota"),]$five_words<-c(paste(as.character(wordlist3[['Minnesota']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="mississippi"),]$five_words<-c(paste(as.character(wordlist3[['Mississippi']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="missouri"),]$five_words<-c(paste(as.character(wordlist3[['Missouri']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="nebraska"),]$five_words<-c(paste(as.character(wordlist3[['Nebraska']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="new jersey"),]$five_words<-c(paste(as.character(wordlist3[['New Jersey']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="new mexico"),]$five_words<-c(paste(as.character(wordlist3[['New Mexico']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="new york"),]$five_words<-c(paste(as.character(wordlist3[['New York']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="north carolina"),]$five_words<-c(paste(as.character(wordlist3[['North Carolina']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="ohio"),]$five_words<-c(paste(as.character(wordlist3[['Ohio']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="oklahoma"),]$five_words<-c(paste(as.character(wordlist3[['Oklahoma']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="pennsylvania"),]$five_words<-c(paste(as.character(wordlist3[['Pennsylvania']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="south carolina"),]$five_words<-c(paste(as.character(wordlist3[['South Carolina']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="tennessee"),]$five_words<-c(paste(as.character(wordlist3[['Tennessee']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="texas"),]$five_words<-c(paste(as.character(wordlist3[['Texas']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="vermont"),]$five_words<-c(paste(as.character(wordlist3[['Vermont']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="virginia"),]$five_words<-c(paste(as.character(wordlist3[['Virginia']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="washington"),]$five_words<-c(paste(as.character(wordlist3[['Washington']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="west virginia"),]$five_words<-c(paste(as.character(wordlist3[['West Virginia']][c(1:5),c(1)]),collapse=', '))
states[c(states$region=="wisconsin"),]$five_words<-c(paste(as.character(wordlist3[['Wisconsin']][c(1:5),c(1)]),collapse=', '))

republican<-c('alabama|alaska|arizona|arkansas|florida|georgia|idaho|indiana|iowa|kansas|kentucky|louisiana|michigan|mississippi|missouri|montana|nebraska|north carolina|north dakota|ohio|oklahoma|pennsylvania|south carolina|south dakota|tennessee|texas|utah|west virginia|wisconsin|wyoming') #states that were republican in the 2016 election
states$political<-ifelse(str_detect(states$region,republican)==TRUE,'Republican','Democrat') #ifelese statement to label states by politics

ctrdata<-df2[c('state','CTR')] #subsets wanted columns
ctrdata$state<-tolower(ctrdata$state) #makes the column all lowercase
statectr<-ctrdata%>%group_by(state)%>%summarise(averagectr=mean(CTR)) #gets the average ctr by state
colnames(statectr)<-c('region','averagectr') #renames the columns
statectr$averagectr<-round(statectr$averagectr,digits=8) #rounds the ctr
bb<-join(states,statectr,by='region',type='left') #joins the two dataframes
bb[which(is.na(bb$averagectr)==TRUE),]$averagectr<-0 #makes all NA ctr into 0
bb$averagectr<-percent(bb$averagectr) #makes ctr into percent
bb$region<-stringr::str_to_title(bb$region) #makes the first letter capitalized

#make the grah
hope2<-ggplot(data=bb,aes(text=paste('State:',region,'<br>Common Words:',five_words,'<br>2016 Electoral Party:',political,'<br>Average CTR:',averagectr),fill=political)) + 
  geom_polygon(data=bb, aes(x = long, y = lat, group = group), color = "white") + 
  coord_fixed(1.4) + 
  scale_fill_manual(name='2016 Electoral Vote',values=c('Republican'='red','Democrat'='blue')) +
  theme_void()

p<-ggplotly(hope2) #make the graph interactive

htmlwidgets::saveWidget(p,"index.html") #downloads the graph as html

##### Paul's Pre-Work, Analysis and More #####
library(tidycensus)
library(stringr)
library(ggplot2)
library(dplyr)
